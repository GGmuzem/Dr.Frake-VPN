...
QString LanguageModel::getCurrentSiteUrl(const QString &path)
{
    ...
    case LanguageSettings::AvailableLanguageEnum::Russian:
        return "https://fblink-sc.com" + (path.isEmpty() ? "" : (QString("/%1").arg(path)));
    default: return QString("https://fblink-sc.com") + (path.isEmpty() ? "" : (QString("/%1").arg(path)));
    }
}
...
QString LanguageModel::getCurrentDocsUrl(const QString &path)
{
    ...
    switch (language) {
    case LanguageSettings::AvailableLanguageEnum::Russian:
        return "https://frakebit.com/docs" + (path.isEmpty() ? "" : (QString("/%1").arg(path)));
